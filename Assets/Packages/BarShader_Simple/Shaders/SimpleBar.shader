...
Shader "Bar/Simple" {
	Properties {
		...
		_Fill ("Fill Amount (_Fill)", Range(0, 1)) = 1
		...
		_Delta ("Delta Fill (_Delta)", Range(0, 1)) = .5
		...
		Pass {
			CGPROGRAM
				...
				fixed4 frag(v2f IN, UNITY_VPOS_TYPE screenPos : VPOS) : SV_Target {
					...
					if(p < ft) { blendColor = _CoinColor; }
					else if (p < fn) { blendColor = _DollarColor; }
